...
class TestMain(unittest.TestCase):
    ...
    def test_main_write_filled_expressions_when_output_is_defined(self):
        ...
        main.main("Link: &ENC[test]ODE& &TOKEN_0& - Auth:&TOKEN_1&/&TOKEN_2&", token_paths,
    ...
    def test_parse_args_return_all_parameters(self):
        assert main.parse_args(["pattern"]) == ("pattern", [], None, None)
        assert main.parse_args(["pattern", "-t", "/path/tokenA/", "/path/tokenB.txt"]) == \
               ("pattern", ["/path/tokenA/", "/path/tokenB.txt"], None, None)
        assert main.parse_args(["pattern", "-t", "/path/tokenA/", "/path/tokenB.txt", "-e", "base64"]) == \
               ("pattern", ["/path/tokenA/", "/path/tokenB.txt"], "base64", None)
        args_with_output = main.parse_args(["pattern", "-o", "/output"])
        ...
        args_with_output_and_max_rows = main.parse_args(["pattern", "-o", "/output", "-om", "50"])
